...
### GET `locations/?kind=provinces|districts|providers&parent=<id>`
...
provider_name, district_name, interval_seconds, days_ahead, desired_bookings,
...
### POST `watchers/` body `{ provider_id, district_id, province_id, provider_name, mode, interval_seconds?, days_ahead?, desired_bookings?, notify? }` → `{ watcher }`
...
### GET `preferences/` → `{ notifications_enabled, sound_enabled, email_on_booking, email_address }`
